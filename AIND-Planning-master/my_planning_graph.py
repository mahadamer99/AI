...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level-1]:
            for s_node in a_node.effnodes:
                states_nodes.add(s_node)
                a_node.children.add(s_node)
                s_node.parents.add(a_node)
        self.s_levels.append(states_nodes)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_node in node_a1.parents:
            for a2_node in node_a2.parents:
                if a1_node.is_mutex(a2_node):
                    return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for index_level, node_s_set in enumerate(self.s_levels):
                node_s=PgNode_s(goal,True)
                if node_s in node_s_set:
                    level_sum += index_level
                    break
